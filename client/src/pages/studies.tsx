...
const Studies = (): ReactElement => {
  ...
  const getAllStudies = (): void => {
    firebase.firestore().collection("studies").onSnapshot((doc) => {
      const data: AppStudyWithoutID[] = [];
      doc.forEach((d: any) =>
        data.push(d.data())
      );
      console.log("arryof data", data);
  ...
  return (
    <div>
      TOP BAR
      <br />
      <p style={{ fontWeight: "bolder" }}>Trending</p><br />
      ...
      <p style={{ fontWeight: "bolder" }}>You may be interested in</p><br />
      {
